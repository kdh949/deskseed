...
- Resource constraints: a runtime field is visible/editable only in the server-projected form and its global actor capability; Platform surface is deliberately not expanded in this first contract slice.
...
- all required admin and core runtime operation families as `FROZEN` reviewer-visible contracts: #83 provides ADMIN fields/forms, reconciled #87 provides ADMIN tags/statuses, and this final runtime stacked PR promotes customer/agent operations only after their mappings, authorization, audit, failure semantics, and tests are present.
- V40 plus additive V41 persistence foundation and ADMIN vertical slices: typed field definition/stable single-select option lifecycle, versioned form draft/preview/validation/publish/archive with one published default per actor, normalized tag catalog with reverse assignment index, and category-compatible custom status catalog; `If-Match` precondition, CSRF/session authorization, Foundation `WorkflowCatalog` condition contribution, and atomic Admin/Security audit.
- agent runtime vertical slice: server-projected typed field values, active tag associations, and compatible custom-status selection travel through the existing ticket command/replay/optimistic-version/TicketAudit/outbox boundary; configuration-only changes emit `ticket.updated` but status/SLA events only follow an actual status transition; access-audited agent read returns no semantic `TICKET_VIEWED`.
...
- publish rejects rule cycles and contradictory final effects; server re-evaluates a submitted form snapshot and atomically rejects a second published customer or agent default.
