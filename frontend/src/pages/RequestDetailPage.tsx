  ...
  if (!Number.isSafeInteger(ticketNumber) || ticketNumber <= 0) {
    return (
      <section className="narrow-panel">
        ...
        <Link className="button primary" to="/lookup">
          문의 조회로 이동
        </Link>
      </section>
    )
    ...
    return (
      <section className="narrow-panel">
        <div className="error-banner" role="alert">
          <strong>
            {error?.problem?.title ?? '문의 내용을 불러오지 못했습니다.'}
          </strong>
          <span>{error?.message ?? '잠시 후 다시 시도해 주세요.'}</span>
          {error?.problem?.requestId && (
            <small>요청 ID: {error.problem.requestId}</small>
          )}
        </div>
  ...
  return (
    <section className="ticket-page">
      ...
      <aside className="notice-card">
        ...
        <p>
          상담사 공개 답변과 고객 추가 댓글은 M2/P1에서 연결됩니다. 내부 메모는
          이 화면과 API에 노출되지 않습니다.
        </p>
      </aside>
    </section>
  )
