...
export function NewRequestPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => saveRequestToken(result.ticketNumber, result.accessToken),
  })
  ...
  if (mutation.data) {
    ...
    return (
      <section className="narrow-panel success-panel">
        ...
        <p>
          이 브라우저에는 조회 키를 저장했습니다. 아래 키는 다른 기기에서 조회할 때 필요하며,
          서버에서 원문을 다시 복구할 수 없습니다.
        </p>
        ...
        <div className="button-row">
          <Link className="button primary" to={`/requests/${submitted.ticketNumber}`}>
            문의 내용 보기
          </Link>
          <button
  ...
  return (
    <section className="form-layout">
      <div>
        ...
        <h1>무엇을 도와드릴까요?</h1>
        <p className="muted">
          현재는 로그인 없이 접수할 수 있습니다. 답변 확인을 위한 조회 키가 발급됩니다.
        </p>
      </div>
      <form className="support-form" onSubmit={submit}>
        {apiError && (
          <div className="error-banner" role="alert">
            ...
            {apiError.problem?.requestId && <small>요청 ID: {apiError.problem.requestId}</small>}
          </div>
        )}
