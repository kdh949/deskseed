...
type ComposerDrafts = Record<TicketVisibility, { body: string; attachmentIds: string[] }>
...
export function useTicketDraftSync({
  ...
  useEffect(() => {
    ...
    const recover = async () => {
      ...
      await Promise.all(
        (Object.keys(CHANNELS) as TicketVisibility[]).map(async (visibility) => {
          ...
          if (newest && !editorAlreadyContainsDraft) {
            ...
            if (newest === remote) versions.current[visibility] = newest.draftVersion
