...
describe('CustomerRequestCreatePage', () => {
  ...
  it('submits through createCustomerRequest and stores its one-time proof only in session storage', async () => {
    ...
    expect(await screen.findByText('/requests/submitted/1042')).toBeVisible()
    expect(
      sessionStorage.getItem('deskseed:customer-request-access:1042'),
    ).toBe(accessToken)
