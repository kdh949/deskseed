...
- 1~2자 literal 검색은 immutable `staff_ticket_search_ngrams(staff_document)`의 `u:`/`b:` token GIN 표현식 인덱스로 후보를 줄인 뒤 같은 escaped `LIKE`로 재검증한다. 결과·exact count 의미는 바뀌지 않으며 index는 projection과 함께 rebuildable하다.
