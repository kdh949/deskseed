...
- Remote CI gap: the earlier #87 head `8cf489c` failed Backend tests in `ApiDocumentationIntegrationTest` because `FROZEN` runtime operations lacked mappings. The staged replacement `bd2cd15` passed the forced local route gate, but GitHub had not registered a run for that head at this checkpoint; this evidence commit requests a fresh PR synchronize. Until that run is green, this PR remains Draft.
