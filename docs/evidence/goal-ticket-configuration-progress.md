...
- Stack reconciliation: merge commit `88215ed` absorbs #87 without rewriting shared history. This commit promotes only the four implemented customer/agent runtime operations from `BLUEPRINT_READY` to `FROZEN`; the staff blueprint registry entries are removed because the frozen operations carry their direct actor contract bindings.
