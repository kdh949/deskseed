...
| C — HMAC and SSRF boundary | `WebhookSignatureSigner` signs timestamp + raw body; `WebhookTargetValidator` validates every DNS answer and endpoint-scoped private allowlist; secret is encrypted with versioned AES-GCM material | Passed: `WebhookSecurityContractTest`, `AdminWebhookIntegrationTest` ciphertext assertion | Resolver result must be bound to the transport adapter |
| B — endpoint/delivery durable model | V60 adds endpoint, encrypted versioned secret, subscription, delivery and attempt tables; `(endpoint_id,event_id)` is unique | Passed: `AdminWebhookIntegrationTest` create/rotation/test/replay | HTTP worker state/configuration remains next slice |
| D — outbox materialization | `WebhookEventSerializer` is a provider seam; the v1 ticket serializer only produces deterministic metadata for PUBLIC events and `JdbcWebhookEventMaterializer` inserts one delivery intent per enabled matching subscription | Passed: `WebhookOutboxMaterializerIntegrationTest` confirms two-endpoint fan-out, uniqueness and INTERNAL exclusion | bounded virtual-thread HTTP transport, retry/circuit/lease recovery/dead-letter transition remain unimplemented |
