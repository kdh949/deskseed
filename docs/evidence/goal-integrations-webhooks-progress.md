...
|---|---|---|---|
| A — admin webhook contract | `30-integrations.yaml` owns FROZEN lifecycle, secret rotation, test, delivery history/detail, replay and terminal archive operations; archive has no hard delete path and detail exposes only redacted attempt status/latency/error metadata; deterministic bundle refreshed | Passed: bundle/ownership/documentation check, `ApiDocumentationIntegrationTest` runtime route inventory, and `AdminWebhookIntegrationTest` non-leak/archive lifecycle assertions | pagination is deferred; raw request/response headers and bodies remain intentionally unavailable |
