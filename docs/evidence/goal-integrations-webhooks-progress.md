...
|---|---|---|---|
| A — admin webhook contract | `30-integrations.yaml` owns FROZEN lifecycle, secret rotation, test, delivery history/detail, replay and terminal archive operations; endpoint `deliverySummary` is status-only aggregate/last failure metadata with no payload, headers, body, secret, or provider diagnostics; archive has no hard delete path and detail exposes only redacted attempt status/latency/error metadata; deterministic bundle refreshed | Passed: bundle/ownership/documentation check, `ApiDocumentationIntegrationTest` runtime route inventory, and `AdminWebhookIntegrationTest` non-leak/archive/summary assertions | pagination is deferred; raw request/response headers and bodies remain intentionally unavailable |
...
| B — endpoint/delivery durable model | V60 adds endpoint, encrypted versioned secret, subscription, delivery and attempt tables; V61 adds endpoint version and bounded delivery/circuit lease state; V63 adds nullable terminal `archived_at` plus a default-list index; `(endpoint_id,event_id)` is unique | Passed: `AdminWebhookIntegrationTest` applies Flyway through V63 and proves inactive-only archive, pending cancellation, history retention and `WEBHOOK_ENDPOINT_ARCHIVED` audit; Flyway-backed worker tests remain green | Custom non-secret headers and endpoint-specific timeout/retry administration are deferred; global bounded settings are active |
| D — worker | PUBLIC event fan-out is durable; a bounded Java virtual-thread batch claims with PostgreSQL `FOR UPDATE SKIP LOCKED`, signs outside transactions, records safe attempt metadata, retries retryable HTTP/network failures, dead-letters at max attempts, recovers expired leases, and persists endpoint circuit state. Claim SQL excludes inactive and archived endpoints; archive cancels pending/retry intents while already leased work is never recalled. | Passed: `WebhookOutboxMaterializerIntegrationTest`, `WebhookDeliveryWorkerIntegrationTest` success/retry/dead-letter/circuit/lease coverage against V63, `ArchitectureTest` | Real receiver transport/redirect/DNS-pinning E2E and load evidence are Not run; no success is claimed for them |
...
| BE archive lifecycle | Passed | Fresh `AdminWebhookIntegrationTest` applies Flyway through V63 (3 tests, zero failures/errors); isolated `WebhookDeliveryWorkerIntegrationTest` (4 tests) and `ArchitectureTest` (1 test) also passed. An initial API documentation test before deterministic bundle refresh failed only because the generated Core outline was stale; the refreshed, isolated `ApiDocumentationIntegrationTest` passed (4 tests, zero failures/errors). |
| BE endpoint delivery summary | Passed | `AdminWebhookIntegrationTest` proves empty, cancelled and dead-lettered status counts plus the latest failure category/timestamp. The projection reads only delivery status/timestamps/error category; it never selects payload, request/response headers, bodies or secret material. `ApiDocumentationIntegrationTest` confirms the bundled HTTP contract. |
