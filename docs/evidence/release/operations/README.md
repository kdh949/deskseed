...
Default full mode pulls public base images through a task-owned anonymous Docker client configuration and passes `--no-cache` to the build; this bypasses cache reuse for those build steps but does not delete existing Docker build cache. The runner pins the exact local Unix daemon, Docker CLI and plugins, applies bounded pull/build timeouts to the whole command process group, and refuses inherited registry-auth, remote-builder and telemetry overrides. The evidence generator intentionally writes outside the repository by default. It attempts exact owned-resource cleanup, verifies the run projects' containers/networks/volumes, run-scoped images, anonymous client configuration and secret directory are absent, records that result, and only then writes evidence. Review the result for accidental sensitive data before adding it here.
