...
| Strict audit/auth/conflict/idempotency regressions | PASS | included in the 127-test clean backend run: stable 503/non-leak, denied-read zero success audit, actor mismatch command immutability, mixed-conflict atomicity, exact/concurrent replay and misuse 409 |
...
| Release-scale performance | PASS | V1-V14, 100k Customer/1M Ticket/2M Comment, 10 exact plans, all five View p95 values 0.608-10.167 ms against the predeclared 50 ms local budget, 33 source hashes/six freeze checkpoints, and PERF-003 PASS; exact cleanup verified |
| Install/upgrade/backup/restore | LIMITED (current-image path PASS) | final isolated full rehearsal completed in 139s: no-cache build, fresh V11 install, V11→V14 in 5s, 93,929-byte `pg_dump` in 403ms, fresh restore parity in 341ms, post-restore application smoke in 8s, role denials, exact cleanup and final source freeze all PASS; no prior tagged binary exists, so formal OPS-001 remains LIMITED |
