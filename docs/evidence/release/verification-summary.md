...
| Visual/axe/keyboard automation | PASS | final frontend source passed Chromium 41/41 and Firefox 41/41 locally, plus WebKit 35/35 + 6/6 in the official Playwright 1.62.1 Noble image; 0 retry/skip. The local macOS WebKit page-fixture timeout is recorded as an environment limitation; see `ui/automated-and-visual-review.md` |
| Browser-assisted direct visual/keyboard review | PASS | Codex in-app browser loaded all 84 Darwin/Linux baselines with 0 broken images and reviewed the complete gallery. Live fixture checks covered 1440px and 390px reflow, 200% page scale, `prefers-reduced-motion: reduce`, workspace separator keyboard resize (`300`→`316`) and arrow-key tab selection; no horizontal overflow occurred in the 390px public form |
| Screen-reader review | NOT RUN | VoiceOver/NVDA reading order and announcements were not run by explicit owner decision; automated axe and DOM semantics are not presented as a screen-reader substitute |
...
| Operations gate accounting | LIMITED / NOT MET | OPS-001 is LIMITED because no previous tagged binary exists; OPS-002 passes only for the local synthetic scope; OPS-003 and OPS-005 remain LIMITED. The owner deferred OPS-004 readiness/alerts/central logging to a future public-production release, so it remains NOT MET and this portfolio RC makes no production-stable claim |
| npm advisory baseline | PASS | `npm audit --omit=dev` and `npm audit`; 0 vulnerabilities on the final lockfile |
| Backend/container/license baseline | LIMITED | Root `LICENSE` records the owner-approved MIT project license; third-party notices remain separate. Gradle runtime graph and deterministic 265-location license audit PASS. A final PR #22 clean-archive no-cache rebuild produced backend `sha256:bf488702…` and frontend `sha256:c8835134…`; exact scan-only images and temporary paths were removed. Scout CVE/SPDX was NOT RUN because package-metadata egress was not owner-authorized and no installed local-only scanner was available, so the verdict remains UNKNOWN |
