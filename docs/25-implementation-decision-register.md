...
| D-065 | Personal-staging observability preserves the `production` profile and adds an opt-in private Prometheus/OTLP logs-and-traces overlay without Docker socket, host collector, profiler, or monitoring-server mutation | accepted | multi-instance topology, log-source boundary, private network policy, or live ingestion evidence changes |
...
| D-067 | AI draft rewrite accepts only an unexpired server-generated PUBLIC reply job reference, reuses its answer and canonical citations without ticket/KB retrieval, and fails back to the original reply when fact/citation preservation is not established | accepted | staff-edited input provenance approval, expanded style/language evaluation, or measured preservation/cost failure; ADR 0050 |
| D-068 | AI provider prompt caching uses an explicit repository-owned static prefix only, bypasses prefixes below 1,024 reviewed tokens without padding, records provider usage buckets, and remains independently disabled until total-cost evidence supports activation | accepted | provider API/pricing/TTL change, a reviewed eligible prefix, retention-policy conflict, or measured cost/quality regression; ADR 0051 |
| D-069 | AI PUBLIC KB embeddings use exact model-snapshot/dimension/normalization/final-input artifacts, bounded synchronous arrays, and a separately disabled durable offline Batch lifecycle with current-source revalidation and provider-file cleanup | accepted | immutable model identity, provider array/Batch/pricing/file-retention contract, PUBLIC KB data approval, or measured quality/cost/cleanup evidence changes; ADR 0052 |
| D-070 | AI PUBLIC reply sent attribution uses a server-owned origin candidate, result-read body-free binding, atomic comment intent, and idempotent post-commit delivery without storing answer or diff telemetry | accepted | cross-actor/shared draft requirement, attribution retention policy, multi-source contribution metric, or measured command/read-path overhead; ADR 0053 |
