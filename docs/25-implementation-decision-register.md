...
| D-060 | Customer authentication throttling uses a storage-neutral port with one Redis adapter; the product owner waived the PostgreSQL comparison benchmark to keep malicious limiter I/O and lock pressure off the transactional database, while the declared 20 req/s sustained, 100 req/s 60-second burst, concurrency 100, 2x safety, p95 20 ms/p99 50 ms, ordinary p95 degradation 10%, and zero-error targets remain unmeasured release evidence | accepted | target traffic/SLO changes, supported-deployment evidence, or Redis operating model changes |
| D-061 | Customer Portal and Staff Console use independent frontend applications, builds, design systems, token namespaces, Storybooks, and machine-enforced import/style boundaries while sharing only the production origin and immutable Deskseed brand sources | accepted | operational evidence favors separate deployments, or a reviewed shared non-visual runtime contract is required |
...
| D-064 | Load observability uses private Prometheus pull, Alloy-pushed logs/traces, always-on JFR wall profiles linked to sampled root spans through the Boot-owned OTel provider, queryid-only PostgreSQL statistics, load-only protected plan capture, and separate-host k6 remote write; allocation/lock profiling, Pushgateway and production rollout remain excluded by default | accepted | production topology, collector security boundary, profiler overhead, or measured load evidence changes |
| D-065 | Personal-staging preserves the `production` profile and baseline private Prometheus/OTLP overlay; a second explicit diagnostics overlay may add root-span profiling plus separately operated least-privilege DB/host collectors, while Docker-socket collection, personal-staging EXPLAIN ANALYZE, default production enablement and automatic monitoring-server mutation remain excluded | accepted | multi-instance topology, log-source boundary, private network policy, profiler overhead, or live ingestion evidence changes |
| D-066 | AI V1 alone uses an isolated FastAPI/worker codebase, separate PostgreSQL+pgvector and Redis Streams with two durable outbox boundaries while Deskseed Backend retains current authorization, PUBLIC-only projection, audit and settings ownership | accepted | AI removal, measured operational-cost failure, topology/scale evidence, or provider/data-residency policy change; ADR 0049 |
| D-067 | Staff ticket search returns a relation-aware count from `limit + 1`, rejects unfiltered non-numeric terms shorter than three code points, and bounds each PostgreSQL search statement to five seconds while preserving authorization and strict audit | accepted | measured first-page latency/recall failure, asynchronous exact-count requirement, or approved external search-store decision; ADR 0050 |
| D-068 | Staff search adds separate mutable non-`CLOSED` and immutable `CLOSED` projections with transactional movement, bounded resumable backfill and reconciliation while the V35 read path remains the rollback source | accepted | `CLOSED` becomes mutable/reopenable, measured write or backfill overhead is unsafe, or split-read evidence rejects the design; ADR 0051 |
| D-069 | Unfiltered staff search reads active matches before terminal matches, preserves the selected sort within each partition, and binds the partition in signed cursor v3; explicit status reads only its owning split projection | accepted | product requires one global active-plus-terminal score order, a total request budget replaces per-statement budgets, or measured split-read evidence rejects the design; ADR 0052 |
| D-070 | Decimal-integer staff search is exact ticket-number lookup; Staff Console defaults to latest-first and the updated-time SQL path omits relevance scoring while score order remains explicit opt-in | accepted | measured latest-first latency still misses the user goal, numeric text-reference search is required, or relevance-quality evidence requires a different default; ADR 0053 |
| D-071 | Latest-first staff search adds an unfiltered `(updated_at desc, ticket_number desc) include (id)` ticket index as a measured PostgreSQL planner option while preserving trigram matching, membership, cursor, permission, timeout and audit contracts | accepted | exact-SHA personal-staging evidence rejects the path, write/storage cost is unacceptable, or a later progressive-search contract supersedes it; ADR 0054 |
