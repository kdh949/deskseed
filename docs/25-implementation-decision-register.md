...
| D-057 | Authenticated staff presence is advisory and single-instance only; a declared multi-instance production topology fails fast until a separately approved shared-delivery design has evidence | accepted | an operator needs multi-instance presence with a reviewed shared delivery boundary |
