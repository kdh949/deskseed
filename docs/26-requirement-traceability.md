...
| REQ-COL-002 | 읽기 권한이 있는 상담사는 단일 인스턴스 범위의 authenticated WebSocket presence와 commit 뒤 safe stale 알림을 받고, presence는 ticket lock이나 optimistic concurrency를 대체하지 않는다 | BLUEPRINT_READY | Wave 1 D2 | ADR 0042, 31, 33, 34, 39, 50, 55 | Origin/session/ticket authorization, message rate·size, heartbeat TTL, disconnect cleanup, after-commit only notification, no body/PII payload, multi-instance fail-fast, frontend contribution and real-stack two-agent verification |
