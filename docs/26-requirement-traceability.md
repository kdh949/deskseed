...
| REQ-SRCH-001 | PostgreSQL 검색으로 시작하고 측정 후 Elasticsearch로 확장한다 | IMPLEMENTATION_READY | P6/P9 | 03, 11, 32, 47, ADR 0050 | frozen POST search contract, SQL authorization, relation-aware `limit + 1` count, cumulative score/ticketNumber stable cursor, short-unfiltered-query validation and five-second SQL budget, query protection/audit with count relation, V35 versioned staff-only PUBLIC/INTERNAL-separated trigram projection and transactional refresh/rebuild lock, literal wildcard and Korean/English/internal/exact-rank corpus, fixed query-count/index-plan tests, real-stack E2E, 1M personal-staging p50/p95/outcome evidence; `/agent/search` raw-query URL exclusion, server filters, opaque cursor history, exact/lower-bound UI and origin search-event detail handoff unit/Storybook |
