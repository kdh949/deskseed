...
| REQ-SRCH-001 | PostgreSQL 검색으로 시작하고 측정 후 Elasticsearch로 확장한다 | IMPLEMENTATION_READY | P6/P9 | 03, 11, 32, 47, ADR 0050/0051/0052/0053 | frozen POST search contract, SQL authorization, relation-aware `limit + 1` count, partition/snapshot/tuple-bound signed cursor v3, short-unfiltered-query validation and five-second per-statement SQL budget, query protection/audit with count relation, V35 rollback projection, V95 active/immutable-CLOSED projections with transactional movement, bounded resumable backfill and zero-gap reconciliation, V96 split trigram indexes, active-before-terminal read order and explicit-status single-partition reads, exact-only decimal-integer lookup, latest-first default without relevance-score evaluation, literal wildcard and Korean/English/internal corpus, fixed query-count/index-plan tests, real-stack E2E, 1M personal-staging p50/p95/outcome evidence; `/agent/search` raw-query URL exclusion, server filters, opaque cursor history, exact/lower-bound UI and origin search-event detail handoff unit/Storybook |
