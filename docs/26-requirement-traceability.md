...
| REQ-CONSENT-002 | registration과 request submission은 final transaction에서 current required policy versions를 검증하고 append-only acceptance를 account/ticket mutation과 원자적으로 저장한다 | BLUEPRINT_READY | P0 consent | 21, 23, 25, 33, 34, 39, 56, D-058 | Identity accepted-policy inputs와 current-policy projection contract는 동결됨. Form-aware request submission/acceptedPolicies contract가 Task 3에서 동결돼야 `IMPLEMENTATION_READY`; missing/duplicate/stale/wrong-context rejection, server time, atomic audit/acceptance, body-free logs/audits; CONSENT-002, DOC-001 |
...
| REQ-CFG-014 | 고객 문의는 server-authorized form/version의 customer-safe projection과 typed candidate values를 사용하고 최종 제출에서 현재 form snapshot을 다시 검증·보존한다 | BLUEPRINT_READY | P0 request form | 21, 25, 26, 33, 34, 39, 47, 56, ADR 0041, D-059 | Core OpenAPI freeze 후 `IMPLEMENTATION_READY`; customer/staff existence-leak regression, stale/current version, conditional projection, typed normalization, selected-form persistence, one TicketAudit; CFG-006, TKT-001/002, CHG-001/002/003, DOC-001 |
