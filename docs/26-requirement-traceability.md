...
| REQ-CFG-003 | 상담사가 versioned PERSONAL/SHARED macro를 preview한 뒤 하나의 command로 적용한다 | IMPLEMENTATION_READY | Wave 2 | 33, 34, 39, 45, 47, ADR 0024, ADR 0040 | `MacroDefinitionMigrationTest`, `MacroDefinitionIntegrationTest`의 owner/shared capability/immutable version/ETag/activation/audit rollback 및 preview no-ticket-side-effect/template allowlist/`MACRO_PREVIEWED` fail-closed access audit; 후속 apply one-command-one-audit 검증 |
