...
| REQ-AUT-001 | 티켓 이벤트 조건 기반 trigger를 제공한다 | BLUEPRINT_READY | P4 | 12, 34, 45, ADR 0024 | `TriggerEngineMigrationTest`, `TriggerDefinitionIntegrationTest`의 typed allowlist/immutable version/ETag/activation·rollback target 검증/reorder/admin capability/audit rollback/dry-run zero-side-effect, 모든 `TicketSubmitted` root transaction의 ordered active-version snapshot job과 insert failure 전체 rollback; 후속 worker command·loop control·webhook provenance로 AUT-001~008 완료 |
