...
|---|---|---:|---|---|---|
| REQ-KB-001 | 고객·상담사·관리자가 Category→Section→Article과 immutable revision lifecycle을 audience에 맞게 사용한다 | IN_PROGRESS | Wave 1 | Goal 03, 02, 03, 25, 32, 33, 34, ADR 0013, 0018, 0040 | hierarchy/FK/order/slug·revision/publish conflict·public/agent/admin lifecycle PostgreSQL tests; 관리자 계층/초안/검토/발행 UI, section list와 return-to-draft 회귀 및 Storybook MCP |
| REQ-KB-002 | 상담사는 PUBLIC·INTERNAL·selected group 문서를 권한으로 검색·열람하고 결과 열람은 required access audit 뒤에 반환한다 | IN_PROGRESS | Wave 1 | Goal 03, 19, 33, 34, ADR 0018 | audience matrix, restricted detail/search audit failure injection, no count/snippet leakage tests; 상담 지식 검색/읽기/링크 삽입 UI, 삽입 전 audience 재검증과 공개/내부 초안 보존 story |
...
| REQ-AUTH-003 | 고객은 password registration과 email verification을 거쳐 로그인하고 single-use reset으로 credential과 기존 session을 안전하게 교체한다 | IMPLEMENTATION_READY | P0 auth | 21, 23, 25, 33, 34, 37, 39, 52, 56, ADR 0042/0043, D-060 | `requestCustomerRegistration`, `verifyCustomerRegistration`, `createCustomerPasswordSession`, `requestCustomerPasswordReset`, `resetCustomerPassword`, `getCurrentCustomer` runtime contracts와 documentation contract tests; 여섯 operation은 runtime parity 후 `FROZEN`; 2026-09-11 고객 가입 UI에서 약관별 수락, 목적별 이메일 안내, browser-bound verification 화면과 실패 복구 연결 (`CustomerRegistrationFlow.test.tsx`), 400 정책 변경 복구와 서버 코드 포인트 입력 검증 및 typed 약관 렌더링 회귀; ADR 0043의 owner-selected Redis 단일 adapter와 atomic TTL/fail-closed 검증, declared target/SLO는 supported-deployment 부하 검증 전까지 `Not run`; [비밀번호 재설정 UI](tasks/2026-09-11-customer-password-reset.md) `CustomerPasswordResetPages.test.tsx`와 Storybook의 입력/만료/일시 장애·재요청 상태 회귀; AUTH-005/006/007, ARCH-004, MAIL-001/002, DOC-001 |
| REQ-AUTH-004 | magic-link login은 passwordless identity에만 허용하고 명시적 registration completion은 password/profile/current consent를 원자적으로 설정하되 이메일로 티켓을 claim하지 않는다 | IMPLEMENTATION_READY | P0 auth | 21, 23, 25, 33, 34, 37, 39, 52, 56, ADR 0029/0042 | `requestCustomerMagicLink`, `consumeCustomerMagicLink`, `completePasswordlessCustomerRegistration` runtime contracts와 `CustomerMagicLinkAuthIntegrationTest`, `CustomerPasswordlessRegistrationCompletionIntegrationTest`, documentation contract tests; 세 operation은 runtime parity 후 `FROZEN`; 2026-09-11 passwordless 로그인 후 가입 완료 UI와 세션/CSRF/약관 제출 및 원래 문의 복귀 회귀 연결 (`CustomerAuthenticationCompletion.test.tsx`), expected-customer mismatch 저장 거부 및 401 복구, 별도 이메일 탭·만료 재요청 복귀 Playwright 회귀; AUTH-001/002/003/004/008, ARCH-004, MAIL-001/002, DOC-001 |
...
| ID | 요구사항 | 상태 | 단계 | 기준 문서 | 최소 검증 |
|---|---|---:|---|---|---|
| REQ-CONSENT-001 | ADMIN은 `customer-consent:manage` 권한으로 registration/request-submission 동의 정책을 draft하고 immutable version으로 publish/archive하며 과거 수락 문서를 재현할 수 있다 | IMPLEMENTATION_READY | P0 consent | 21, 23, 25, 33, 34, 39, 56, D-058 | `listCurrentCustomerConsentPolicies`, ADMIN list/create/detail/update/publish/archive implementation contracts, blueprint actor registry와 `CustomerConsentContractTest`의 authority/CSRF/precondition/audit rollback/safe-document/history projection/server-owned immediate effective-published timestamp, 50,000-character/200,000-byte canonical document limit, 262,144-byte HTTP body limit, context별 current policy 20개 publish 직렬화; 미구현 operation은 runtime `FROZEN`을 보류; CONSENT-001, DOC-001 |
| REQ-CONSENT-002 | registration과 request submission은 final transaction에서 current required policy versions를 검증하고 append-only acceptance를 account/ticket mutation과 원자적으로 저장한다 | IMPLEMENTATION_READY | P0 consent | 21, 23, 25, 33, 34, 39, 56, D-058 | 2026-09-08: request submission의 최종 동의 검증/acceptance/security audit와 rollback/replay 테스트 구현 (`docs/tasks/2026-09-08-ticket-form-runtime.md`). Identity와 `CreateCustomerRequest`의 accepted-policy inputs, current-policy projection, stable stale/configuration problem blueprint를 확정함. missing/duplicate/stale/wrong-context rejection, server time, account/ticket mutation과 acceptance/audit의 atomicity, body-free logs/audits; CONSENT-002, CFG-006, DOC-001 |
...
| REQ-AUT-001 | 티켓 이벤트 조건 기반 trigger를 제공한다 | IMPLEMENTATION_READY | P4 | 12, 34, 45, ADR 0024 | `TriggerEngineMigrationTest`, `TriggerDefinitionIntegrationTest`, `TriggerExecutionIntegrationTest`의 typed allowlist/immutable version/ETag/activation·rollback target 검증/reorder/admin capability/audit rollback/dry-run zero-side-effect, 모든 `TicketSubmitted` root transaction의 ordered version snapshot, evolving-state normal command, `TRIGGER_APPLIED` audit, depth/action/fingerprint guard, retry/dead-letter atomic rollback, metadata-only `ticket.trigger.executed` outbox; 2026-09-08 TICKET_UPDATED/CUSTOMER_REPLIED, TAG/FORM/ASSIGNEE 조건, SET_PRIORITY/SET_ASSIGNEE/미배정 그룹 알림 및 관리자 버전·이력·dry-run/활성화 UI 확장. tasks/2026-09-08-trigger-workflow.md |
| REQ-AUT-002 | 시간 경과 기반 automation을 제공한다 | IMPLEMENTATION_READY | P4 | 12, 34, 45, ADR 0024 | `AutomationMigrationTest`, `AutomationDefinitionIntegrationTest`, `AutomationCandidateScannerIntegrationTest`, `AutomationExecutionIntegrationTest`의 versioned solved-age/CLOSE_TICKET allowlist, admin capability, ETag activation history, zero-side-effect dry-run, partial solved index, advisory single-scanner, 100-row batch, `(version,ticket,solvedAt)` interval idempotency, leased machine command/`AUTOMATION_APPLIED`, stale interval skip, retry/dead-letter rollback으로 AUT-009 충족; 관리자 설정·저장 버전·dry-run·활성화·이력 UI는 `tasks/2026-09-08-automation-management.md`; 다른 time condition/action은 후속 범위 |
...
| ID | 요구사항 | 상태 | 단계 | 기준 문서 | 최소 검증 |
|---|---|---:|---|---|---|
| REQ-CFG-001 | 태그와 조건 기반 saved view를 제공한다 | IMPLEMENTATION_READY | P2/P6 | 33, 34, 39, 47, ADR 0041 | 2026-09-08: TAG/FORM/CUSTOM_STATUS/CUSTOM_FIELD와 조회용 비민감 typed equality, fieldKey 및 filter catalog 계약 동결. `docs/tasks/2026-09-08-configuration-view-filters.md`; CFG-003/005, ACC-001, DOC-001, PERF-001 |
| REQ-VIEW-001 | allowlisted condition AST 기반의 versioned SYSTEM/PERSONAL/SHARED saved view를 제공한다 | IMPLEMENTATION_READY | P1 | 33, 34, 39, 47, 50 | PostgreSQL `AgentTicketReadIntegrationTest`의 AST allowlist/owner·shared capability/version conflict/reorder/preview/authorization·audit/description 영속성·동일 compiler row-count/one-UNION-ALL count와 batch 단일 `ticketCountAsOf`; frontend description draft/conflict 보존, `EXACT`/`OMITTED_VISIBLE_LIMIT` count metadata 조합의 strict decoder와 negative tests, exact count 기준 시각, server persistence, columns/sort/share scope and cursor reset unit/Storybook; PERF-001 |
| REQ-CFG-002 | typed custom field와 form을 제공한다 | BLUEPRINT_READY | P6 | 38, 47 | type validation/migration/projection 테스트 |
| REQ-CFG-003 | 상담사가 versioned PERSONAL/SHARED macro를 preview한 뒤 하나의 command로 적용한다 | IMPLEMENTATION_READY | Wave 2 | 33, 34, 39, 45, 47, ADR 0024, ADR 0040 | `MacroDefinitionMigrationTest`, `MacroDefinitionIntegrationTest`의 owner/shared capability/immutable version/ETag/activation/audit rollback, preview no-ticket-side-effect/template allowlist/`MACRO_PREVIEWED` fail-closed access audit, apply field/tag/custom-field/edited-comment one-version-one-audit/exact replay 및 `MACRO_APPLIED` provenance; 개인/공유 매크로 편집·명시적 활성화 UI 및 권한 적용 history GET, TKT-006/CHG-001 |
| REQ-CFG-010 | 관리자가 immutable machine key와 typed EAV validation을 가진 ticket field definition/option을 관리하고, 서버 projection이 customer/staff field visibility를 분리한다 | IMPLEMENTATION_READY | Wave 1 | 02 Goal, 32, 33, 39, 47, ADR 0041 | PostgreSQL type CHECK/option lifecycle/visibility-bypass/audit rollback, owned Core contract and customer/staff projection tests; ADMIN `/admin/ticket-fields` 생성·편집·활성화·선택지 UI와 `AdminTicketFieldsPage.stories.tsx` 입력 보존/denied/MSW CSRF 검증; CFG-001, UI-002/004; 2026-09-08 상담사 field read/candidate/edit UI와 required/form identity 회귀 (`docs/tasks/2026-09-08-agent-ticket-configuration.md`) |
| REQ-CFG-011 | 관리자가 immutable published version과 server-authoritative conditional visibility를 가진 ticket form을 관리한다 | IMPLEMENTATION_READY | Wave 1 | 02 Goal, 33, 34, 39, 47, ADR 0041 | form publish cycle/contradiction/hidden-required/version-stale integration tests and customer/agent projection E2E; ADMIN `/admin/ticket-forms` 필드 배치·조건 편집·초안 저장·발행·보관 UI와 `AdminTicketFormsPage.stories.tsx` conflict/If-Match 검증; CFG-002, UI-002/004; 상담사 기본 폼의 조건 판정과 draft/conflict/replay UI 연결 |
| REQ-CFG-012 | 관리자가 normalized tag catalog을 관리하고, ticket command·검색·저장형 View에서 tag를 권한과 감사 경계 안에 사용한다 | IMPLEMENTATION_READY | Wave 1 | 02 Goal, 33, 34, 39, 47, ADR 0041 | 관리자 태그 CRUD와 상담사 add/remove UI 및 저장/감사 회귀 구현. Saved View 조건은 후속; normalization/concurrent add-remove/audit/authorization/query contributor tests; CFG-003 |
| REQ-CFG-013 | 관리자가 fixed status category에 매핑되는 custom status label을 관리하며 CLOSED terminal compatibility를 보존한다 | IMPLEMENTATION_READY | Wave 1 | 02 Goal, 33, 34, 39, 47, ADR 0041 | 관리자 상태 label/category/form 제한 편집과 상담사 상태 선택 UI 구현; default uniqueness/status-category compatibility/closed mutation denial/old-client category integration tests; CFG-004 |
| REQ-CFG-014 | 고객 문의는 server-authorized form/version의 customer-safe projection과 typed candidate values를 사용하고 최종 제출에서 현재 form snapshot을 다시 검증·보존한다 | IMPLEMENTATION_READY | P0 request form | 21, 23, 25, 26, 32, 33, 34, 39, 47, 56, ADR 0041, D-059 | 2026-09-08: customer projection/candidate/접수와 모바일 UI 구현, 폼 snapshot·receipt·동의/첨부 원자성 검증 (`docs/tasks/2026-09-08-ticket-form-runtime.md`). `projectCustomerTicketForm`, closed customer projection/value schemas, form-aware JSON/multipart `CreateCustomerRequest`, stable existence-safe problem blueprint를 확정함. 구현은 CUSTOMER_REQUEST 고정 public projection, field/option semantic snapshot과 current display copy, deterministic selection backfill, customer/staff existence-leak, stale/current version, typed normalization, stable initial-command replay/conflict/single-winner, planned-customer multipart rollback, one TicketAudit를 검증; CFG-006, TKT-001/002, CHG-001/002/003, FILE-001/003/004/006, DOC-001 |
| REQ-FILE-001 | private object storage 기반 첨부파일을 제공한다 | IMPLEMENTATION_READY | P1 | 38, 39, 48, 50 | 기존 PostgreSQL attachment pipeline과 production `S3AttachmentObjectStoreIntegrationTest`의 VersityGW bounded multipart exact-byte/read/delete, `AttachmentProductionBoundaryTest`의 local adapter 제외·acknowledged `UPSTREAM_WAF` source/no-rescan, `AttachmentMultipartHttpBoundaryIntegrationTest`의 11 MiB HTTP upload, `AttachmentCleanupBoundaryTest`의 claim/delete/complete failure ordering; Compose private object-storage network/volume과 20 MiB/file·105 MiB/request 계약. Sophos 장비 rule과 DB+세 object-volume 일관 restore는 별도 운영 증거; FILE-001/003/004/006 |
| REQ-FILE-002 | rich text는 versioned structured JSON, server-derived plain text, attachment-backed image와 safe renderer를 사용하고 redaction은 별도 권한 workflow로 남긴다 | IMPLEMENTATION_READY | P8 | 33, 34, 39, 48, ADR 0045 | closed node/mark decoder, JSON bound, link protocol, XSS corpus, PUBLIC/INTERNAL attachment 격리, customer-safe projection, canonical hash/replay, draft/macro compatibility; FILE-002/003/004 |
...
| REQ-AI-001 | 상담사는 PUBLIC 대화만 사용한 요약·분류 제안·공개 KB 기반 답변 초안을 비동기 API로 생성하고 사람이 검토한다 | IN_PROGRESS | AI V1 | 19, 21, 23, 26, 33, 34, 39, AI design v1.2, ADR 0049/0053 | A~G 서버 gate; 2026-09-17 Agent Workspace AI panel에서 최근 작업·poll/cancel·typed result·feedback과 freshness 재검증 후 명시적 PUBLIC draft 추가/교체를 연결 (`docs/tasks/2026-09-17-ai-v1-frontend-assistant.md`); S03b는 stable candidate/result-read binding과 PUBLIC comment commit의 body-free sent 귀속을 서버→UI 순서로 구현 (`docs/tasks/2026-09-19-ai-cost-s03b-sent-attribution-server.md`, `docs/tasks/2026-09-19-ai-cost-s03b-sent-attribution-ui.md`); S08b는 기본 exact 재사용과 명시적 새 후보·한도·모호한 create 재시도 UI 계약을 고정 (`docs/tasks/2026-09-19-ai-cost-s08b-reuse-ui.md`); live model·사람 평가는 별도 |
...
| REQ-AI-005 | 관리자는 typed enable/stop/status/reindex 정책을 감사 가능하게 관리하고 feedback·보존·관측·평가·복구 근거를 운영한다 | IN_PROGRESS | AI V1 G/S03b/S08 | 19, 21, 23, 36, 52, ADR 0049/0053 | 서버 운영 API·retention·metadata-only 관측·합성 평가 구현; S03b는 inserted/edited interaction과 Backend-confirmed sent를 분리하고 comment/candidate dedupe·unattributed cohort·body-free edit metric을 추가; S08는 explicit reuse/new-candidate intent, 24시간 제한과 UI retry/status 계약을 연결 (`docs/tasks/2026-09-19-ai-cost-s08a-reuse-intent.md`, `docs/tasks/2026-09-19-ai-cost-s08b-reuse-ui.md`); external receipt/deploy와 실제 attribution coverage·절감은 미검증, AI-OPS-001/AI-OBS-001/AI-RET-001/AI-USE-001 |
| REQ-AI-006 | 상담사는 같은 티켓의 아직 유효한 서버 생성 PUBLIC 답변 초안을 닫힌 한국어 문체·길이 옵션으로 재작성하고, 사실·부정 의미·인용 보존이 확인되지 않으면 원 답변을 유지한다 | IN_PROGRESS | AI V1 S12 | 21, 23, 26, 33, 34, 39, ADR 0049/0050 | `ticket.reply_rewrite` sourceJob binding, required source-result access audit, no ticket/KB re-retrieval, exact citation map, two-call upper bound와 preservation failure fallback 구현; source 원문 유지·닫힌 옵션·별도 preview·명시적 PUBLIC 삽입 UI 계약 (`docs/tasks/2026-09-19-ai-cost-s12-rewrite-ui.md`); AI-API-001, AI-COST-001, AI-REPLY-001, AI-REWRITE-001, UI-002~006 |
| REQ-AI-007 | AI provider prompt cache는 고객/티켓/KB 내용을 제외한 명시적 static prefix만 사용하고, 최소 길이·쓰기/read/일반 입력/출력 비용과 UNKNOWN을 함께 계측해 독립적으로 활성화한다 | IMPLEMENTATION_READY | AI V1 S13 | 21, 23, 26, ADR 0049/0051 | `off | test | intent` mode, 1,024-token eligibility without padding, pinned LiteLLM transport fixture, content-free receipt metadata와 total-cost comparison; AI-COST-001, AI-PROMPT-CACHE-001, AI-OBS-001 |
| REQ-AI-008 | PUBLIC KB embedding은 exact model snapshot·dimension·normalization·final input artifact만 재사용하고 missing input의 bounded array와 offline Batch를 별도 lifecycle·SYSTEM budget·source revalidation·file cleanup 아래 실행한다 | IMPLEMENTATION_READY | AI V1 S14 | 21, 23, 25, 26, 34, ADR 0049/0052 | reusable artifact key/vector binding, complete indexed array mapping과 call-level usage, durable Batch manifest/custom ID/partial/cancel/late result/file cleanup; AI-COST-001, AI-KB-001, AI-EMBED-001, AI-BATCH-001, AI-OPS-001 |

## 9. 프론트엔드 경험

| ID | 요구사항 | 상태 | 단계 | 기준 문서 | 최소 검증 |
|---|---|---:|---|---|---|
| REQ-UI-001 | Deskseed Agent Workspace의 고밀도 Queue/Workspace UI를 제공한다 | IMPLEMENTATION_READY | M2~ | 28, 29, 30, 39 ADR, 55 | canonical `SeedWorkspace` 기반 실제 Queue/Workspace route, 1280·1440·1920 browser baselines, `2026-08-29-staff-console-design-qa.md` |
| REQ-UI-002 | Views 목록과 티켓 테이블을 제공한다 | IMPLEMENTATION_READY | M2 | 28, 30, 55 | `AgentViewsPage.test.tsx`, `07-screens-agent-views-page--queue` Storybook interaction/a11y, canonical surface legacy-import boundary |
| REQ-UI-003 | 좌측 속성·중앙 대화·선택 가능한 우측 context panel 구조를 제공한다 | IMPLEMENTATION_READY | M2 | 29, 30, 39 ADR | `SeedTicketWorkspaceShell`, `AgentTicketEditorWorkspace` Storybook interaction/a11y, `workspace-{1280,1440,1920}.png` browser baselines |
...
- 2026-09-19 REQ-AI-006: [S12 PUBLIC 답변 초안 재작성](tasks/2026-09-19-ai-cost-s12-public-draft-rewrite.md), [S12 재작성 UI](tasks/2026-09-19-ai-cost-s12-rewrite-ui.md). 서버 생성 원 답변 job reference만 입력으로 허용하고 사실·citation 보존 실패 시 원 답변 UI를 유지하며, 검증된 별도 preview만 명시적으로 PUBLIC 작성기에 반영한다. staff-edited 입력, 영어, live 품질 증거는 별도 결정이다.
- 2026-09-19 REQ-AI-007: [S13 provider prompt cache](tasks/2026-09-19-ai-cost-s13-provider-prompt-cache.md). explicit static prefix와 1,024-token eligibility를 동결하며 현재 짧은 prompt는 padding 없이 bypass한다. live hit/cost evidence와 production activation은 별도다.
- 2026-09-19 REQ-AI-008: [S14a embedding artifact/array](tasks/2026-09-19-ai-cost-s14a-embedding-reuse-batching.md), [S14b offline Batch](tasks/2026-09-19-ai-cost-s14b-offline-embedding-batch.md). mutable model alias는 production reuse snapshot으로 인정하지 않고, offline file/result lifecycle과 SYSTEM 비용을 interactive 경로에서 분리한다. live provider·paid reindex·절감과 품질 증거는 별도다.
