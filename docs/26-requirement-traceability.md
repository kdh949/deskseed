...
| REQ-OPS-001 | 격리된 load 환경은 기존 private monitoring server에 bounded metrics, safe structured logs, sampled traces, root-span-linked CPU/wall profiles와 queryid/보호 실행계획 근거를 제공하고 public product surface와 감사 원장을 분리한다 | IN_PROGRESS | Operations/load | 03, 21, 23, 25, 36, ADR 0047 | code/Compose/dashboard/datasource/privacy contract implemented; private management/exporter reachability, trace-profile live ingest, protected plan capture, on/off overhead and OPS-004 live evidence remain required |
| REQ-OPS-002 | personal-staging은 `production` profile을 유지한 채 baseline private Prometheus/OTLP와 별도 opt-in diagnostics profiler·least-privilege DB/host collector를 제공하고 Docker socket/host-wide log 수집 없이 public product surface와 감사 원장을 분리한다 | IN_PROGRESS | Operations/personal-staging | 03, 21, 23, 25, 36, ADR 0048, D-065 | code/Compose/collector/rule/dashboard/datasource/privacy contract implemented; private allowlist, live scrape/Loki/Tempo/Pyroscope ingest, overhead and OPS-004 evidence remain required |
...
| REQ-SRCH-001 | PostgreSQL 검색으로 시작하고 측정 후 Elasticsearch로 확장한다 | IMPLEMENTATION_READY | P6/P9 | 03, 11, 32, 47, ADR 0050/0051/0052/0053/0054 | frozen POST search contract, SQL authorization, relation-aware `limit + 1` count, partition/snapshot/tuple-bound signed cursor v3, short-unfiltered-query validation and five-second per-statement SQL budget, query protection/audit with count relation, V35 rollback projection, V95 active/immutable-CLOSED projections with transactional movement, bounded resumable backfill and zero-gap reconciliation, V96 split trigram indexes, V97 latest-first ticket-order index, active-before-terminal read order and explicit-status single-partition reads, exact-only decimal-integer lookup, latest-first default without relevance-score evaluation, literal wildcard and Korean/English/internal corpus, fixed query-count/index-plan tests, real-stack E2E, 1M personal-staging p50/p95/outcome evidence; `/agent/search` raw-query URL exclusion, server filters, opaque cursor history, exact/lower-bound UI and origin search-event detail handoff unit/Storybook |
...
| REQ-AI-001 | 상담사는 PUBLIC 대화만 사용한 요약·분류 제안·공개 KB 기반 답변 초안을 비동기 API로 생성하고 사람이 검토한다 | IN_PROGRESS | AI V1 | 19, 21, 23, 26, 33, 34, 39, AI design v1.2, ADR 0049 | A~G 서버 gate; 2026-09-17 Agent Workspace AI panel에서 최근 작업·poll/cancel·typed result·feedback과 freshness 재검증 후 명시적 PUBLIC draft 추가/교체를 연결 (`docs/tasks/2026-09-17-ai-v1-frontend-assistant.md`); live model·사람 평가는 별도 |
