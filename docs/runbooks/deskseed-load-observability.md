...
Required versions are Prometheus 3.14, Loki 3.7, Tempo 2.10, Pyroscope 2.2, Grafana 13.2, Alloy 1.18.0, and k6 2.0.0. The Pyroscope Java agent is pinned to 2.9.1 with SHA-256 `4b9aa1ba327a9ce2b1ce062578ad6b876c0d2496f15a00f0fdc4c953638cb76d`; ingest compatibility with the 2.2 server must pass before a load run is accepted.
...
The load-only database enables `pg_stat_statements` and `track_io_timing`. Existing database volumes require the operator to run `CREATE EXTENSION IF NOT EXISTS pg_stat_statements;` once after restart.
...
- the monitoring server can reach the VPN-bound `9090`, `9100`, `8081`, `9187`, `9121`, and `12345` ports;
...
2. Merge the six scrape jobs from `prometheus.yml.example` into the existing Prometheus configuration and replace `deskseed-load.internal` with the load host VPN name/address.
...
6. Reload Prometheus and verify every `up{environment="load",stack="deskseed"}` series is `1`.
...
Compare incoming rate, success/failure counters, oldest queued work, leases, and transport latency. A committed ticket mutation is not rolled back by delivery failure.
