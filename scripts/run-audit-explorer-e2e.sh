...
e2e_work_dir="$(mktemp -d "$e2e_temp_root/deskseed-audit-e2e.XXXXXX")"
...
cleanup() {
  ...
  if [[ "$e2e_work_dir" == "$e2e_temp_root"/deskseed-audit-e2e.?????? ]]; then
...
e2e_initialize_resource_identity audit "$e2e_work_dir"
compose_files=(
...
if DESKSEED_BACKEND_PORT="$backend_port" \
  ...
  DESKSEED_RUNTIME_USER="$e2e_runtime_user" \
...
if [[ "$compose_up_status" -ne 0 ]]; then
  ...
  printf 'Audit Explorer E2E Compose startup failed.\n' >&2
  exit "$compose_up_status"
fi
if [[ "$capture_status" -ne 0 ]] || ! e2e_assert_expected_stack_captured; then
  printf 'Audit Explorer E2E could not prove exact run ownership for the started stack.\n' >&2
  exit 1
fi
