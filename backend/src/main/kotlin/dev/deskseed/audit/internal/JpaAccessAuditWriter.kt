...
internal class JpaAccessAuditWriter(
    ...
    override fun appendTicketResourceRead(event: TicketResourceReadAccessAudit) {
        require(event.actorType == ActorType.STAFF) { "Ticket resource reads require a staff actor" }
        require(event.source == RequestSource.AGENT_UI) { "Ticket resource reads require AGENT_UI source" }
        jdbcTemplate.update(
            """
