...
class CustomerRegistrationMailMigrationTest {
    @BeforeEach
    fun cleanDatabase() {
        flyway().clean()
    }
    ...
    @Test
    fun `V81 point one clean migration allows the registration verification template`() {
        migrateTo("81.1")

        connection().use { connection ->
            connection.createStatement().use { statement ->
                assertThat(templateConstraint(statement))
                    .contains("CUSTOMER_REGISTRATION_VERIFICATION")
                    .contains("CUSTOMER_MAGIC_LINK", "REQUEST_RECEIVED", "PUBLIC_AGENT_REPLY")
                assertThat(
                    statement.executeQuery(
                        "select count(*) from flyway_schema_history where version = '81.1' and success",
    ...
    @Test
    fun `V81 upgrade applies the registration template constraint without changing mail rows`() {
        migrateTo("81")
        connection().use { connection ->
            connection.createStatement().use { statement ->
                statement.executeUpdate(
                    ...
                    values ('00000000-0000-4000-8000-000000008301', 'migration-mail',
                            '<migration-mail@deskseed.invalid>', 'CUSTOMER_MAGIC_LINK', 1,
                            'sender@example.test', 'recipient@example.test', 'Synthetic', 'Synthetic',
                            'QUEUED', 'SYSTEM', 'CUSTOMER_PORTAL', 'request-migration',
                            'correlation-migration', 'command-migration', 0, 5, now(), now(), 0)
                    """.trimIndent(),
                )
            }
        }
        ...
        connection().use { connection ->
            connection.createStatement().use { statement ->
                assertThat(templateConstraint(statement)).contains("CUSTOMER_REGISTRATION_VERIFICATION")
                assertThat(
                    statement.executeQuery("select template_key from outbound_mail_intents where id = '00000000-0000-4000-8000-000000008301'")
                        .use { result -> check(result.next()); result.getString(1) },
                ).isEqualTo("CUSTOMER_MAGIC_LINK")
            }
        }
    }
