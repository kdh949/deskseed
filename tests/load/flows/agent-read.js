...
import { randomUuid, targetUrl } from '../lib/config.js';
import { unexpectedStatus } from '../lib/metrics.js';
import { staffHeaders, staffSession } from '../lib/staff.js';

export function agentReadFlow() {
  staffSession();
  ...
  const search = http.post(
    ...
    JSON.stringify({ query: __ENV.STAFF_SEARCH_QUERY || 'loadtest', filters: {}, sort: 'score:desc,ticketNumber:desc', limit: 25 }),
    {
